...
$.fn.dataTable.ext.search.push(
    function( settings, data, dataIndex ) {
        ...
        //var man = document.getElementById('max').value;
        
        var minage = parseFloat( data[2] ) || 0; // use data for the age column
        var maxage = parseFloat( data[3] ) || 0; // use data for the age column
 
        if ( ( isNaN( min ) && isNaN( max ) ) ||
             ( isNaN( min ) && maxage <= max ) ||
             ( min <= minage   && isNaN( max ) ) ||
             ( min <= minage   && maxage <= max ) )
        {
            return true;
        }
        return false;
